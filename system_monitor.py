...
def monitor_processes(stdscr):
    ...
    while True:
        ...
        display_bar(stdscr, disk_usage_percent, "Disk  ", 2, width)
        ...
        try:
            ...
            stdscr.addstr(4 + len(core_usages) + 1, 0, "Commands: [f] Filter by Name | [p] Filter by PID | [s] Toggle Sort CPU/Mem | [a] Toggle Asc/Desc | [k] Kill Process".ljust(width - 1))

            # Search Bars
            search_bar_name = "Search Process Name: "
            stdscr.addstr(5 + len(core_usages) + 1, 0, search_bar_name.ljust(width - 1))
            stdscr.addstr(5 + len(core_usages) + 1, len(search_bar_name), filter_name.ljust(width - len(search_bar_name)))

            search_bar_pid = "Filter PID: "
            stdscr.addstr(6 + len(core_usages) + 1, 0, search_bar_pid.ljust(width - 1))
            stdscr.addstr(6 + len(core_usages) + 1, len(search_bar_pid), str(filter_pid) if filter_pid else "None".ljust(width - len(search_bar_pid)))
        except curses.error:
        ...
        header_start_row = 7 + len(core_usages)
        try:
        ...
        try:
            ...
            if key == ord('q'):
                break
            elif key == ord('f'):  # Filter by Name
                curses.echo()
                stdscr.addstr(5 + len(core_usages) + 1, len(search_bar_name), " " * (width - len(search_bar_name)))  # Clear previous input
                stdscr.addstr(5 + len(core_usages) + 1, len(search_bar_name), "Enter Process Name: ")
                name_input = stdscr.getstr(5 + len(core_usages) + 1, len(search_bar_name) + 20, 20).decode('utf-8')
                filter_name = name_input.strip()
                curses.noecho()
                last_message = ""
            elif key == ord('p'):  # Filter by PID
                curses.echo()
                stdscr.addstr(6 + len(core_usages) + 1, len(search_bar_pid), " " * (width - len(search_bar_pid)))  # Clear previous input
                stdscr.addstr(6 + len(core_usages) + 1, len(search_bar_pid), "Enter PID: ")
                pid_input = stdscr.getstr(6 + len(core_usages) + 1, len(search_bar_pid) + 12, 10).decode('utf-8')
                ...
                filter_name = ""  # Reset name filter on sort change
                filter_pid = None  # Reset PID filter on sort change
